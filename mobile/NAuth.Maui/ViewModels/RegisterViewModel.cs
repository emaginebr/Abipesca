    ...
    public class RegisterViewModel : BaseViewModel
    {
        ...
        private async Task RegisterAsync()
        {
            await ExecuteAsync(async () =>
            {
                ...
                {
                    await ShowErrorAsync("Nome deve ter no m�nimo 3 caracteres");
                    return;
                }

                if (string.IsNullOrWhiteSpace(Email) || !IsValidEmail(Email))
                {
                    await ShowErrorAsync("E-mail inv�lido");
                    return;
                }

                if (string.IsNullOrWhiteSpace(Password) || Password.Length < 8)
                {
                    await ShowErrorAsync("Senha deve ter no m�nimo 8 caracteres");
                    return;
                }

                if (!HasLettersAndNumbers(Password))
                {
                    await ShowErrorAsync("Senha deve conter letras e n�meros");
                    return;
                }

                if (Password != ConfirmPassword)
                {
                    await ShowErrorAsync("As senhas n�o coincidem");
                    return;
                }
